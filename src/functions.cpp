...
string readinput() {
    try {
        ...
        if (!std::getline(cin, choice_s)) {
            throw std::runtime_error(INVALID_CHOICE);
        }
        ...
        if (choice == 1) {
            ...
            if (!std::getline(cin, file_choice_s)) {
                throw std::runtime_error(INVALID_CHOICE);
            }
            // ignoruoju tarpus vartotojo ivedime
            file_choice_s.erase(std::remove(file_choice_s.begin(), file_choice_s.end(), ' '), file_choice_s.end());
            ...
            std::ifstream infile(std::string("files/") + file_choice_s);
            if (!infile) {
                throw std::runtime_error(FILE_OPEN_ERROR + file_choice_s + ".");
            }
            string line;
